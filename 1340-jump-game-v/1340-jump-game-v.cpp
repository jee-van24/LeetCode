...
public:
    int solve(int idx, vector<int>&arr,vector<int>&dp,int d){
        if(dp[idx]!=-1){
            //already reached this idx and i have the best answer stored already so dont recompute the same state
            return dp[idx];
        }
        int temp1=0,temp2=0;
        for(int j=idx+1;j<=min((int)arr.size()-1,idx+d);j++){
            ...
            temp1=max(temp1,1+solve(j,arr,dp,d));
        }
        for(int j=idx-1;j>=max(0,idx-d);j--){
            if(arr[j]>=arr[idx])break;
            temp2=max(temp2,1+solve(j,arr,dp,d));
        }
